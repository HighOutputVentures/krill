...
test('rabbitmq, given single worker with multiple messages', async (t) => {
  ...
  await Adapter.RabbitMQ.subscribe('sample.worker2', async ({ body }) => {
    const time = 2500 * Math.random();
    await delay(time);
    return { time, ...body };
  });
  const result = await Promise.all(_.times(5, async () => Adapter.RabbitMQ.publish('sample.worker2', message)));
...
test('rabbitmq, given a timeout request', async (t) => { t.pass(); });
